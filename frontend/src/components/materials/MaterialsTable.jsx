...
function MaterialRow({ material, onDownload, onMore }) {
  const hasActions = onDownload || onMore;

  return (
    <tr className="hover:bg-surface-container-lowest transition-colors group">
      <td className="px-sp-md py-2">
      ...
      <td className="px-sp-md py-2 text-label-md font-label-md text-on-surface-variant">
        {material.course}
      </td>
      <td className="px-sp-md py-2 text-label-md font-label-md text-on-surface-variant">
        {material.dateAdded}
      ...
      <td className="px-sp-md py-2 text-right">
        {hasActions ? (
            ...
            <tr>
              {TABLE_HEADERS.map((col) => (
                <th
                  key={col || 'actions'}
                  className={cn(
                    'px-sp-md py-2 font-label-sm text-label-sm text-on-surface-variant uppercase tracking-wider',
                    col === 'Name' && 'w-1/3'
                  )}
                >
                  {col}
